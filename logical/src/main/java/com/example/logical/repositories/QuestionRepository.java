...
public interface QuestionRepository extends JpaRepository<Question, Long> {
    @Modifying
    @Transactional
    @Query(value = "UPDATE question SET question_name = :value WHERE question_id = :id", nativeQuery = true)
    public void updateQuestionName(@Param("value") String value, @Param("id") Long id);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM question WHERE question_id = :id", nativeQuery = true)
    public void deleteQuestion(@Param("id") Long id);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO question (question_id, question_name, question_answer, answer_true) VALUES (:id, :question_name, :question_answer, :answer_true)", nativeQuery = true)
